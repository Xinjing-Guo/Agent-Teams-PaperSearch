...
After generating the review markdown, render it to PDF using Pandoc + XeLaTeX:
...
pandoc <review>.md \
  ...
  -V title="<Review Title>" \
  -V author="Generated by Agent-Teams-PaperSearch" \
  -V date="<current date>" \
  --number-sections \
  --toc
```
...
- Use LaTeX math notation for subscripts/superscripts in the markdown (e.g., `$_2$` not `₂`, `$^+$` not `⁺`)
- Requires `pandoc` and `xelatex` installed on the system
- If XeLaTeX is unavailable, fall back to `--pdf-engine=pdflatex` (but avoid Unicode subscripts)
